...
class DeviceManager(object):
    ...
    def update_device_list(self, timeout=200):
        ...
        if (timeout < 0) or (timeout > UNSIGNED_INT_MAX):
            logger.debug("DeviceManager.update_device_list: "
    ...
    def update_all_device_list(self, timeout=200):
        ...
        if (timeout < 0) or (timeout > UNSIGNED_INT_MAX):
            logger.debug("DeviceManager.update_all_device_list: "
                  "timeout out of bounds, timeout: minimum=0, maximum=%s" % hex(UNSIGNED_INT_MAX).__str__())
            return 0, None
    ...
    def open_device_by_index(self, index, access_mode=GxAccessMode.CONTROL):
        ...
        if index < 1:
            logger.debug("DeviceManager.open_device_by_index: index must start from 1")
            return None
        elif index > UNSIGNED_INT_MAX:
            logger.debug("DeviceManager.open_device_by_index: index maximum: %s" % hex(UNSIGNED_INT_MAX).__str__())
            return None
        ...
        if access_mode not in access_mode_dict.values():
            logger.debug("DeviceManager.open_device_by_index: "
    ...
    def open_device_by_sn(self, sn, access_mode=GxAccessMode.CONTROL):
        ...
        access_mode_dict = dict((name, getattr(GxAccessMode, name)) for name in dir(GxAccessMode) if not name.startswith('__'))
        if access_mode not in access_mode_dict.values():
            logger.debug("DeviceManager.open_device_by_sn: "
                  "access_mode out of bounds, %s" % access_mode_dict.__str__())
            return None
    ...
    def open_device_by_user_id(self, user_id, access_mode=GxAccessMode.CONTROL):
        ...
        access_mode_dict = dict((name, getattr(GxAccessMode, name)) for name in dir(GxAccessMode) if not name.startswith('__'))
        if access_mode not in access_mode_dict.values():
            logger.debug("DeviceManager.open_device_by_user_id: access_mode out of bounds, %s" % access_mode_dict.__str__())
            return None
    ...
    def open_device_by_ip(self, ip, access_mode=GxAccessMode.CONTROL):
        ...
        access_mode_dict = dict((name, getattr(GxAccessMode, name)) for name in dir(GxAccessMode) if not name.startswith('__'))
        if access_mode not in access_mode_dict.values():
            logger.debug("DeviceManager.open_device_by_ip: access_mode out of bounds, %s" % access_mode_dict.__str__())
            return None
    ...
    def open_device_by_mac(self, mac, access_mode=GxAccessMode.CONTROL):
        ...
        access_mode_dict = dict((name, getattr(GxAccessMode, name)) for name in dir(GxAccessMode) if not name.startswith('__'))
        if access_mode not in access_mode_dict.values():
            logger.debug("DeviceManager.open_device_by_mac: access_mode out of bounds, %s" % access_mode_dict.__str__())
            return None
...
class IntFeature(Feature):
    ...
    def get_range(self):
        ...
        implemented = self.is_implemented()
        if not implemented:
            logger.debug("%s.get_range is not support" % self.feature_name)
            return None
    ...
    def get(self):
        ...
        if not readable:
            logger.debug("%s.get is not readable" % self.feature_name)
            return None
...
class FloatFeature(Feature):
    ...
    def set(self, float_value):
        ...
        if not check_ret:
            logger.debug("FloatFeature.set: float_value out of bounds, %s.range=[%f, %f]" %
...
class EnumFeature(Feature):
    ...
    def set(self, enum_value):
        ...
        if enum_value not in enum_value_list:
            logger.debug("EnumFeature.set: enum_value out of bounds, %s.range:%s" %
...
class StringFeature(Feature):
    ...
    def get_string_max_length(self):
        ...
        implemented = self.is_implemented()
        if not implemented:
            logger.debug("%s.get_string_max_length is not support" % self.feature_name)
            return None
    ...
    def set(self, input_string):
        ...
        if input_string.__len__() > max_length:
            logger.debug("StringFeature.set: "
...
class BufferFeature(Feature):
    ...
    def get_buffer_length(self):
        ...
        implemented = self.is_implemented()
        if not implemented:
            logger.debug("%s.get_buffer_length is not support" % self.feature_name)
            return None
    ...
    def get_buffer(self):
        ...
        readable = self.is_readable()
        if not readable:
            logger.debug("%s.get_buffer is not readable" % self.feature_name)
            return None
    ...
    def set_buffer(self, buf):
        ...
        writeable = self.is_writable()
        if not writeable:
            logger.debug("%s.set_buffer is not writeable" % self.feature_name)
            return

        max_length = self.get_buffer_length()
        if buf.get_length() > max_length:
            logger.debug("BuffFeature.set_buffer: "
...
class CommandFeature(Feature):
    ...
    def send_command(self):
        ...
        implemented = self.is_implemented()
        if not implemented:
            logger.debug("%s.send_command is not support" % self.feature_name)
            return
...
class DataStream:
    ...
    def set_acquisition_buffer_number(self, buf_num):
        ...
        if (buf_num < 1) or (buf_num > UNSIGNED_LONG_LONG_MAX):
            logger.debug("DataStream.set_acquisition_buffer_number:"
    ...
    def get_image(self, timeout=1000):
        ...
        if (timeout < 0) or (timeout > UNSIGNED_INT_MAX):
            logger.debug("DataStream.get_image: "
                  "timeout out of bounds, minimum=0, maximum=%s"
                  % hex(UNSIGNED_INT_MAX).__str__())
            return None

        if self.acquisition_flag is False:
            logger.debug("DataStream.get_image: Current data steam don't  start acquisition")
            return None
...
class RawImage:
    ...
    def __raw16_to_raw8(self, pixel_bit_depth, valid_bits):
        ...
        if pixel_bit_depth == GxPixelSizeEntry.BPP10:
            ...
            logger.debug("RawImage.__dx_raw16_to_raw8: Only support 10bit and 12bit")
            return None
    ...
    def convert(self, mode, flip=False, valid_bits=DxValidBit.BIT4_11,
        ...
        if self.frame_data.status != GxFrameStatusList.SUCCESS:
            logger.debug("RawImage.convert: This is a incomplete image")
            return None
        ...
        if convert_type not in convert_type_dict.values():
            logger.debug("RawImage.convert: convert_type out of bounds, %s" % convert_type_dict.__str__())
            return None
        ...
        if valid_bits not in valid_bits_dict.values():
            logger.debug("RawImage.convert: valid_bits out of bounds, %s" % valid_bits_dict.__str__())
            return None
        ...
        if pixel_bit_depth < GxPixelSizeEntry.BPP8 or \
           pixel_bit_depth > GxPixelSizeEntry.BPP12:
            logger.debug("RawImage.convert: This pixel format is not support")
            return None

        if mode == "RAW8":
            if flip is True:
                logger.debug('''RawImage.convert: mode="RAW8" don't support flip=True''')
                return None
            ...
            if pixel_bit_depth in (GxPixelSizeEntry.BPP10, GxPixelSizeEntry.BPP12):
                ...
                logger.debug('RawImage.convert: mode="RAW8" only support 10bit and 12bit')
        elif mode == "RGB":
            if pixel_bit_depth in (GxPixelSizeEntry.BPP10, GxPixelSizeEntry.BPP12):
                image_raw8 = self.__raw16_to_raw8(pixel_bit_depth, valid_bits)
            else:
            ...
            logger.debug('''RawImage.convert: mode="%s", isn't support''' % mode)
            return None
    ...
    def get_numpy_array(self):
        ...
        """
        if self.frame_data.status != GxFrameStatusList.SUCCESS:
            logger.debug("RawImage.get_numpy_array: This is a incomplete image")
            return None
...
class Utility:
    ...
    def get_gamma_lut(gamma=1):
        ...
        if (gamma < GAMMA_MIN) or (gamma > GAMMA_MAX):
            logger.debug("Utility.get_gamma_lut: gamma out of bounds, range:[0.1, 10.0]")
            return None

        status, gamma_lut, gamma_lut_len = dx_get_gamma_lut(gamma)
        if status != DxStatus.OK:
            logger.debug("Utility.get_gamma_lut: get gamma lut failure, Error code:%s" % hex(status).__str__())
            return None
    ...
    def get_contrast_lut(contrast=0):
        ...
        if (contrast < CONTRAST_MIN) or (contrast > CONTRAST_MAX):
            logger.debug("Utility.get_contrast_lut: contrast out of bounds, range:[-50, 100]")
            return None

        status, contrast_lut, contrast_lut_len = dx_get_contrast_lut(contrast)
        if status != DxStatus.OK:
            logger.debug("Utility.get_contrast_lut: get contrast lut failure, Error code:%s" % hex(status).__str__())
            return None

...
class Buffer:
    ...
    def from_file(file_name):
        ...
        # logger.debug("data_array_len0:", len(file_string))
        data_array = create_string_buffer(file_string,len(file_string))
        # logger.debug("data_array_len:",len(data_array))
...
class RawImage:
    ...
    def __convert_to_special_pixelformat(self, pixelformat , convert_type, channel_order, pixel_bit_depth, valid_bits, flip):
        ...
        if pixel_bit_depth == GxPixelSizeEntry.BPP8:
            ...
            logger.debug("ImageProc.__convert_to_special_pixelformat: not support")
            return None
    ...
    def __raw8_pixel_format_rotate_90(self, pixel_format, direct):
        ...
        if pixel_format & PIXEL_BIT_MASK != GX_PIXEL_8BIT:
            logger.debug("__raw8_pixel_format_rotate_90.pixel_format only support raw8")
            return -1

        if direct != 90 and direct != -90:
            logger.debug("__raw8_pixel_format_rotate_90.direct only support 90 or -90")
            return -1
    ...
    def __raw8_pixel_format_mirror(self, pixel_format, mirror_mode):
        ...
        """
        if pixel_format & PIXEL_BIT_MASK != GX_PIXEL_8BIT:
            logger.debug("__raw8_pixel_format_mirror.pixel_format only support raw8")
            return -1

        if mirror_mode not in (DxImageMirrorMode.VERTICAL_MIRROR, DxImageMirrorMode.HORIZONTAL_MIRROR):
            logger.debug("mirror_mode only support VERTICAL_MIRROR or HORIZONTAL_MIRROR")
            return -1
...
class Utility:
    ...
    def get_lut(contrast=0, gamma=1, lightness=0):
        ...
        if status != DxStatus.OK:
            logger.debug("Utility.get_lut: get lut failure, Error code:%s" % hex(status).__str__())
            return None
    ...
    def calc_cc_param(color_correction_param, saturation=64):
        ...
        if status != DxStatus.OK:
            logger.debug("Utility.calc_cc_param: calc correction param failure, Error code:%s" % hex(status).__str__())
            return None
    ...
    def calc_user_set_cc_param(color_transform_factor, saturation=64):
        ...
        if status != DxStatus.OK:
            logger.debug("Utility.calc_user_set_cc_param: calc correction param failure, "

...
import torch
from detectron2.config import LazyConfig, CfgNode, instantiate
...
class DetectronDetector:
    def __init__(self, config_dir: str, device: str):
        ...
        with open(osp.join(config_dir, "detectron_detector.yaml"), "r") as f:
            self.config = yaml.safe_load(f)

        self.cfg = LazyConfig.load(osp.join(config_dir, self.config["config"]))
        for i in range(3):
            self.cfg.model.roi_heads.box_predictors[i].test_score_thresh = 0.25

        self.cfg.train.init_checkpoint = self.config["weights"]

        self.model = DefaultPredictor_Lazy(self.cfg)

    def predict(self, image: np.ndarray) -> List[PersonDetection]:
        outputs = self.model(image)
        instances = outputs["instances"]
        human_idx = instances.pred_classes == 0  # Assuming class 0 is human
        human_instances = instances[human_idx]
        bboxes = human_instances.pred_boxes.tensor
        scores = human_instances.scores
        all_data = torch.cat([bboxes, scores.unsqueeze(1)], dim=1)
        ...
        for box in all_data:
            ...
            person = PersonDetection(box_np, score)
...
if __name__ == "__main__":
    import cv2
    detector = DetectronDetector("src/cv_module/configs", "cuda")

    image = cv2.imread("photos/people.jpg")

    people = detector.predict(image)
    ...
    cv2.imwrite("detected_people.jpg", img_to_draw)

...
class SAM2CameraPredictor(SAM2Base):
    ...
    def _run_memory_encoder(self, frame_idx, batch_size, high_res_masks, is_mask_from_pts):
        ...
        - New SAM2Base needs `object_score_logits`
        - Old SAM2Base doesn't accept it
        """
        # Retrieve correct image features
        _, _, current_vision_feats, _, feat_sizes = self._get_image_feature(frame_idx, batch_size)

        try:
            # Derive a per-object scalar logit from mask logits (B×1×H×W -> B×1)
            obj_logits = high_res_masks.mean(dim=(-1, -2)).to(high_res_masks.device, dtype=torch.float32)

            # New signature (with object_score_logits)
            maskmem_features, maskmem_pos_enc = self._encode_new_memory(
                current_vision_feats=current_vision_feats,
                feat_sizes=feat_sizes,
                pred_masks_high_res=high_res_masks,
                object_score_logits=obj_logits,
                is_mask_from_pts=is_mask_from_pts,
            )
        except TypeError:
            # Old signature (no object_score_logits)
            maskmem_features, maskmem_pos_enc = self._encode_new_memory(
                current_vision_feats=current_vision_feats,
                feat_sizes=feat_sizes,
                pred_masks_high_res=high_res_masks,
        ...
        # Offload & normalize return format (unchanged from your code)
        storage_device = self.condition_state["storage_device"]

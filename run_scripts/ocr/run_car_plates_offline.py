...
from src.project_managers import CarPlatesOCRProcessingManager
...
def process(video_path, pos, max_frames, record, use_cpp_api):
    ...
    log_path = Path(f"logs/ocr/{video_path.stem}")
    log_path.mkdir(parents=True, exist_ok=True)
    logger.add(log_path / "logger.log", level="INFO")
    if use_cpp_api:
        try:
    ...
    manager = CarPlatesOCRProcessingManager(obstacles_off=False, roi=[0, 0, 2048, 2048])

    if record:
        fps = 10
        video_log_path = str(log_path / "output.mp4")
        video_out = cv.VideoWriter(str(video_log_path),
                                   cv.VideoWriter.fourcc(*'mp4v'), fps,
                                   (2448, 2048), True)
        assert video_out.isOpened()

    for pixel_tensor in source:
        logger.info('==================================================')
        logger.info(f'{source.i_frame} : pos_frame={source.pos}, pos_sec={source.pos_sec} s')

        frame_out, tms = manager.process(pixel_tensor, source.pos)
...
def main():
    ...
    process(args.video_path, args.pos, max_frames,
            args.record, args.use_cpp_api)
